...
| **Next** | **The chain's wiring.** Attempting R20d.1 found four design gaps in the elements between a well and a sale and all four are now closed — a separator that holds a pressure (157), an S4 that tells a controller from a restriction (158), a registry a caller can ask what it holds, and a `Manifold` that commingles (159). **Every element the chain needs exists and is tested.** What is left is the wiring: stage 4's segment plan, stage 5 calling `IFlowSolver` per segment over the registered network, stage 6 committing duration-weighted withdrawals, custody metered at stage 7 and priced at stage 8, and the direct well→sale path deleted. **Expect the numbers to move** — R20.4 owns the rebalance. |
...
|---|---|---|---|
| R20d.1 | Flow solver — real network, not a direct well→sale path | ✅ R4 | 🟨 **every element the chain needs now exists; the wiring is what is left.** The loop still calls `SolveOperatingPoint` per well. Cleared: a separator holds a pressure and imposes it (157), S4 tells a controller from a restriction (158), the registry can be asked what exists, and a `Manifold` commingles so a field can have a second well (159). What remains is stage 4's segment plan, stage 5 solving per segment, stage 6 committing duration-weighted, and custody metered and priced |

...
| **Design docs** | 24 design + 1 research + **26 phase docs**, 17 catalogue sheets + tech tree, 18 SDDs (000–017). Coherence log: **157 findings**, 61–157 from the code passes. **51 open SDD items** now registered here rather than only in the documents that raised them. |
| **Code status** | 15 engine assemblies, 0 warnings, 0 errors, **817 tests**. The kernel, the contract layer, eleven domain modules and Layer 4 composition are implemented; scenario, activity-mass, VOI, lending and souring contracts are declared in their SDDs and compiled. The composed engine **advances a tick and plays**: a player drills, waits, finds oil or doesn't, produces, declines, and wins or goes broke. Every implemented member traces to a pinned SDD section (F-1). |
...
| **Next** | **The chain (R20d.1–5), continued.** Its first element is unblocked: a separator now holds a pressure and imposes it upstream, without which calling the solver would have made every well flow against atmosphere (finding 157). What remains is the wiring itself — stage 4's segment plan, stage 5 calling `IFlowSolver` per segment over the registered network, stage 6 committing duration-weighted withdrawals, and custody becoming a metered point that stage 8 prices. **Expect the numbers to move**: a well flowing against a real separator is not the well that flows against a hard-coded 15 bar, and R20.4 owns the rebalance. **The activities that CHANGE something remain blocked** — workover, install and abandon each reach a subsystem the loop does not yet call (finding 153). |
...
|---|---|---|---|
| R20d.1 | Flow solver — real network, not a direct well→sale path | ✅ R4 | 🟨 **unblocked, not yet wired.** The loop still calls `SolveOperatingPoint` per well. The first element of the chain is now able to hold the network back: a separator declares an operating pressure, flashes at it and imposes it upstream (finding 157). Until that landed, calling the solver would have made every well flow against atmosphere |

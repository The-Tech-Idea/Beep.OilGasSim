...
| **Balance note** | **Measured, then fixed.** The field is a forty-year arc: build-up, a twelve-year plateau at the export limit, decline, a long tail — with the binding constraint moving separator → tank → reservoir. It was four years and no arc, because the aquifer was 1% of the pore volume. The goal is $600M in a decade, which six wells reach only if the vessel is upgraded ($918M) and not otherwise ($449M). Royalty, tax and a per-tonne lifting cost apply. Still untuned: the price is fixed, gas earns nothing, and the tail has no ending because abandonment does not exist. |
| **Next** | **The tail has no ending — abandonment (R12b.10).** The arc now runs build-up → plateau → decline → tail, and the tail is thirty years of a field producing almost nothing while the standing charge eats the cash it made. A player can shut a well in; they cannot stop paying for the field. That needs R20d.9's abandonment obligation, and it is the one missing piece of the shape rather than an addition to it. Behind it: cargoes as operations (SDD-006 §7), injection-for-support (SDD-002 §9's double-count first), treating, and more arcade slots (S005-3).</br></br>Behind that: The scouting pass is done and recorded (finding 162): a well produces one material because nothing ever said what else comes up the hole, gas needs an air-density constant `PhysicalConstants` does not have, and water has no source because nothing computes a water cut. **Three SDD amendments before any code** — which is this phase's own lesson, applied in advance rather than discovered mid-wire.</br></br>Behind that: R20d.1 is done — **there is now one system for moving material.** Stage 4 plans, stage 5 solves the network, stage 6 commits, stage 7 meters, stage 8 prices what crossed; `WellsState.ProduceOver`, the second path, is deleted rather than bypassed. Adding equipment is adding an element to that network. What is bypassed still: the gas legs and water leg of the separator go nowhere (one material), there is no flowline, no tank, no treating, and the sales spec is empty because nothing can be off-spec yet. Each is an element and a material, not a new mechanism. |
...
| R12b.10 | **Abandon** — plug, decommission, restore | 🟨 **half unblocked.** Finding 153's second reason is gone: opex now scales with the liquid lifted, so a watered-out well genuinely costs more than it earns and stopping it is a real decision. What is still missing is the OBLIGATION to discharge, which is R20d.9's company ledger | Wells, Company (discharges the obligation), HSE (legacy dimension) |
...
| R20d.4 | Water — production, treatment, injection, disposal | ✅ R10 | 🟨 **produced, and disposed of.** The aquifer feeds the compartment, the saturation rises, the S-curve gives the water cut, the stream splits by it and the separator's water leg runs to a disposal well whose `Injectivity` can throttle the whole field. **Injection as pressure support is not** — that is the same element committing a receipt rather than disposing, and SDD-002 §9's double-count has to be settled first. Treatment is not |

...
| **Next** | **The company is now as real as the field.** A run prices its oil at a market that moves and reverts, pays for work at what the cycle charges, books 1P/2P/3P reserves that a crash writes down, wears its plant out by the barrel, accrues the plugging bill as it earns it, and borrows against oil in the ground on a facility whose rate follows how much gas it flares. Two headless clients play the whole arc — explore, drill, be wrong, develop, debottleneck, drown, close — through `ReadModel` + `Commands` alone.</br></br>**Findings 164–172 are one finding wearing nine coats**: a mechanism specified, built or declared, and joined to nothing. The world generator composed and never called; POS with no subject; the play correlation copying rather than sharing; `IBeliefStore` with no re-key; design 04 stage 3's gathering line never built; `IPriceModel` and two RNG streams for a market that never moved; `IReserveBasedLending` waiting on reserves that did not exist. **The lesson**: a passing unit suite is evidence about a component and never about a system — and a test can encode a component's defect as its own setup (finding 168), after which the defect is documented as intended rather than merely unnoticed.</br></br>**Next, and they are the same slice**: a GAS CHAIN. Finding 172 is a penalty with no answer — a company can be charged for flaring and cannot buy its way out, because captured gas has nowhere to go. Gas sales or reinjection gives it one, and the same work makes gas-window basins (SDD-010 steps 1–2) something a company can produce rather than a reason a basin is barren. Behind it: §2.5's contour trace; the surface fabric still in empty lists; cargoes and berths (SDD-006 §7); injection for pressure support (SDD-002 §9's double-count first); treating; and R18's incidents, which are the other two thirds of an ESG record. |

...
| R21.5 | Reference headless client proving the contract is sufficient | ✅ — plays a full game on `ReadModel` + `Commands` alone, asserted to reference no domain module. Found `WellStatusView` missing (the surface said how MANY wells, never WHICH, so no well-level command was aimable) and became the measuring instrument for findings 164/165 |

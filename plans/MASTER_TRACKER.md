...
| **Next** | **The game is playable end to end; what is missing is depth, not joins.** A run now goes: a seed buries structures on a horizon → charge migrates up-dip and fills some of them → regional data gives a vague read on their SIZE (never their contents) → POS decomposes into five factors, three shared with the play → seismic sharpens the ones it can see → a hole resolves presence from truth → a dry hole re-prices the whole play, a discovery carries its beliefs onto the field → the chain is laid to where that field actually is → production, commingling, watering out, debottlenecking, shutting in, plugging, closing. Two headless clients play it through `ReadModel` + `Commands` alone.</br></br>**Findings 164–170 are all closed**, and they were one finding wearing seven coats: *a mechanism built to specification and joined to nothing*. The world generator had been composed and never called; POS had no subject; the play correlation copied instead of sharing; `IBeliefStore` had no re-key though SDD-008 §4 has specified one since R14; the gathering line design 04 stage 3 describes was never built. **The lesson worth carrying**: a passing unit suite is evidence about a component and never about a system, and a test can encode a component's defect as its own setup (finding 168) — after which the defect is not merely unnoticed, it is documented as intended.</br></br>**What is actually next**, in rough order of what a player would feel: **step 3 is done** (R20d.10): burial is drawn per basin, maturity is a depth-band lookup, and a basin whose source never cooked — or cooked past the oil window — is genuinely barren while looking identical from the surface. Steps 1–2 (tectonics, stratigraphy) remain, and with them GAS as something a company can actually produce; §2.5's contour trace, so a closure is its own shape rather than a square of the right area; the surface fabric that is still empty lists (roads by A\*, settlements' second purpose, third-party legacy fields, sensitivity zones); and then the older queue — **reserves** (SDD-009 §4), which now gates two things rather than one: the abandonment provision accrual AND units-of-production depreciation, and which needs Arps type-curves per development template — content this composition does not ship yet, though `Defaults` is where it would go, so this is a content gap and not a design one. R20d.7.7 supplied the other half: a discovered field now carries a BELIEF about its oil in place for the quantiles to be taken from; cargoes and berths as operations (SDD-006 §7), injection-for-support (SDD-002 §9's double-count first), treating, and more arcade slots (S005-3). |

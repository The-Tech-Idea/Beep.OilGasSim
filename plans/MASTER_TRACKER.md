...
| # | Finding |
|---|---|
| 179 | **The shipped field stopped watering out, and nothing noticed.** Measured while calibrating finding 178: water over oil BY MASS at the custody meter runs 0.001 at month 180, 0.004 at 300, 0.012 at 420 and **0.019 at month 479** — two per cent after forty years. A water-drive field with a regional aquifer reaches eighty or ninety. R20.4 measured this same composition drowning — *"oil 61,165 → 27,371 m³/month while water climbs 10,171 → 47,529 t"* — and that arc is gone. Something between then and now took it away: the aquifer became per-compartment and strength-scaled (164), the well's inflow started coming from the rock rather than a fixed constant (170), and produced water is re-injected (R20d.18). **No test asserts the field waters out**, so the late-life story the whole ending is built on — watering out, opex outrunning revenue, shut-in, abandonment — regressed silently while every suite stayed green. It also explains finding 178 completely: BS&W at a 0.005 carry-over only breaches a 0.005 limit once water passes half the liquid by mass, and this field peaks at two per cent, so treating had nothing to do because the FIELD had stopped making water rather than because the spec was miscalibrated. Fixing the calibration would have been fixing the wrong thing.</br></br>**Diagnosis, narrowed — two of three suspects eliminated.** *The aquifer model is sound*: given a reservoir drawn from 30 to 20 MPa over forty years, the shipped aquifer (strength 4, τ 40 y) delivers **49 Mm³ against a 100 Mm³ pore volume** — half a pore volume of water, abundant, with its own pressure still at 27.8 MPa at month 360, so it is nowhere near exhausted. *The accumulation is sound*: `CommitWithdrawal` folds influx into `Cumulative.WaterInflux`, and `WaterSaturation` reads connate plus influx plus injection less produced water. So water is available and the compartment would count it. **What remains is the link between them**: either the running reservoir's pressure never falls far enough to drive the influx the probe assumed, or the water cut derived from Sw through the Corey curve is wrong. **Third measurement: the MODEL waters out and the ENGINE does not.** A compartment driven directly with the shipped field's offtake — 61,000 m³ of oil and 73,000 m³ of voidage a month, the shipped aquifer, water drive — holds pressure at 26.6 MPa after thirty-two years and takes **~53,000 m³ of influx a month**, 73% voidage replacement. That puts Sw near 0.55 by year forty, and the shipped Corey curve turns 0.55 into a water cut around **57%**. The running engine reports **2%**, and no viscosity ratio reconciles the two: matching 2% at Sw 0.55 would need water sixteen times more viscous than oil. **So Sw in the running engine is near connate — the influx is not reaching the compartment's saturation.** The call is present (`InfluxFor` per compartment, passed as `CompartmentWithdrawal.Influx`) and `CommitWithdrawal` folds it into `Cumulative.WaterInflux`, so the loss sits between those two points and nowhere else. That is the whole remaining search space, and it is small |

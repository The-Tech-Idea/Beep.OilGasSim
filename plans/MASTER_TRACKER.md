...
| **Design docs** | 24 design + 1 research + **26 phase docs**, 17 catalogue sheets + tech tree, 18 SDDs (000–017). Coherence log: **212 findings**, 61–212 from the code passes. **48 open SDD items of 61 raised** — all 61 registered below, verified against the SDDs rather than counted by hand. |
...
**Sixty-one are raised across the eighteen SDDs: thirteen closed, forty-eight
...
> **These three numbers are DERIVED, and here is the command, because the last
> set was not** (finding 212). An item is a row `| Sxxx-N |` in an SDD's *Open
> items* section; it is closed if its body is struck through or its trigger cell
> is `✅`/`closed`:
>
> ```bash
> grep -hE "^\| S[0-9]{3}-[0-9]+" plans/sdd/*.md | wc -l          # 61 raised
> ```
>
> Re-run it rather than trusting the sentence above. The previous version of this
> paragraph said fifty-three raised and four closed while the header said
> fifty-one open, and all three were hand-counted prose that nothing re-derived.
> **The consolidation this register performs has no maintenance step** — closing
> an item in an SDD does not update this page, and R20d.12 closed six and opened
> two without touching it, which is how nine items came to be missing from a list
> whose entire purpose is that nothing is missing from it.

...
| **Design docs** | 24 design + 1 research + **26 phase docs**, 17 catalogue sheets + tech tree, 18 SDDs (000–017). Coherence log: **159 findings**, 61–159 from the code passes. **51 open SDD items** now registered here rather than only in the documents that raised them. |
| **Code status** | 15 engine assemblies, 0 warnings, 0 errors, **819 tests**. The kernel, the contract layer, eleven domain modules and Layer 4 composition are implemented; scenario, activity-mass, VOI, lending and souring contracts are declared in their SDDs and compiled. The composed engine **advances a tick and plays**: a player drills, waits, finds oil or doesn't, produces, declines, and wins or goes broke. Every implemented member traces to a pinned SDD section (F-1). |
...
| **Next** | **A manifold (finding 159), then the chain's wiring.** Attempting R20d.1 found four design gaps in the elements between a well and a sale, and three are closed: a separator holds a pressure and imposes it (157), S4 can tell a controller from a restriction (158), and a caller can ask the registry what exists. The fourth is a stop: **commingling is an element's job (FD4) and no manifold is declared**, so a field's second well cannot be connected at all. `IFlowNode` is named in design 01 §C5, priced in catalogue C06 and declared in no SDD — it needs SDD-006 before code, like the three before it. After that the wiring itself: stage 4's segment plan, stage 5 solving per segment, stage 6 committing, custody metered and priced. **Expect the numbers to move** — R20.4 owns the rebalance. |
...
|---|---|---|---|
| R20d.1 | Flow solver — real network, not a direct well→sale path | ✅ R4 | 🟨 **three of four blockers cleared.** The loop still calls `SolveOperatingPoint` per well. A separator now holds a pressure and imposes it (157), S4 tells a controller from a restriction (158), and the registry can be asked what exists. **Blocked on a manifold** (159): FD4 makes commingling an element's job and none is declared, so a second well cannot be connected |

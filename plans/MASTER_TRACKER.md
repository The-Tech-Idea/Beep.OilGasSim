...
| R20d.8 | World generation — basins, plays, traps, terrain | ✅ R15 | 🟨 **the reservoir now governs the game, but nothing generates the reservoir.** Findings 164/165 closed the two places a field's size could not reach the player: the aquifer is the compartment's and sized by it, and export capacity is built rather than stated. A 500e6 m³ field is now measurably a better business than a 50e6 one ($601M against $523M over twenty years). Compartments are still hand-built by `FieldControl` — there are no basins, plays, traps or terrain, and no spatial layout at all, so "where everything is" is unmodelled and POS (R20d.7) still has no subject |

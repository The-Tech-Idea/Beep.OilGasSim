...
| **Design docs** | 24 design + 1 research + **26 phase docs**, 17 catalogue sheets + tech tree, 18 SDDs (000–017). Coherence log: **162 findings**, 61–162 from the code passes. **51 open SDD items** now registered here rather than only in the documents that raised them. |
| **Code status** | 15 engine assemblies, 0 warnings, 0 errors, **840 tests**. The kernel, the contract layer, eleven domain modules and Layer 4 composition are implemented; scenario, activity-mass, VOI, lending and souring contracts are declared in their SDDs and compiled. The composed engine **advances a tick and plays**: a player drills, waits, finds oil or doesn't, produces, declines, and wins or goes broke. Every implemented member traces to a pinned SDD section (F-1). |
...
| R20d.3 | Gas — compression, dehydration, sweetening, NGL, flare | ✅ R9 | ⬜ **scouted** (finding 162): a well produces one material, so the separator's gas leg has nothing to carry. Needs SDD-003 §6's produced-stream composition and an air-density constant in `PhysicalConstants` before code |
| R20d.4 | Water — production, treatment, injection, disposal | ✅ R10 | ⬜ **scouted** (finding 162): `FractionalFlow` is built and tested and nothing computes a water cut — `SubsurfaceState` does not advance water saturation, and exposing one touches SDD-008 §3's truth door |

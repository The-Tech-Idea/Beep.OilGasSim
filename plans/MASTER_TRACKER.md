...
| R20d.4 | Water — production, treatment, injection, disposal | ✅ R10 | ⬜ **the last term of SDD-003 §6.1b.** The form is written and the water leg is structurally zero until a compartment carries a rel-perm curve and advances its saturation — `FractionalFlow` is built, tested and called by nothing. The separator's water leg stays unconnected rather than piped to an element that would receive zero |

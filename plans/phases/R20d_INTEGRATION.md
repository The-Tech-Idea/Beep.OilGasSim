...
| R20d-V11 | A diffused node's effects reach the shared effect state | `DiffusionStage` is constructed exactly as `CapabilitiesModule.Compose` builds it and executed directly: a node whose diffusion is due moves an envelope that `EffectState.EffectiveEnvelope` then reports; a node not yet due contributes nothing; and re-running `Execute` after acquisition does not double the contribution (SDD-005 §4.2's R20d.10e amendment — `TechnologyState.ActiveEffects()` existed and nothing called it before this join) |
| R20d-V12 | Land classifies into real terrain, not a sea/land stub | A generated basin's land cells resolve to one of the four terrain classes reachable without a per-cell climate signal, more than one class actually appears, sea is never classified, and the shipped `content/terrain-classes/` set partitions the (height, slope) grid with no gap and no overlap (catalog C16's amendment, finding 242) |
| R20d-V13 | Generation declares the basin's one climate region, and severity reaches it | `IWorldSink.AddClimateRegion` is called with a region covering the generated grid; `EngineBuilder.CreateNew`/`SaveGame`'s regeneration both seal `WeatherState` with `Defaults.Climate` scaled by `WorldParameters.ClimateSeverity`, so a severity of zero collapses temperature and severity to their seasonal baseline exactly, and a reload reproduces the same scaled climate rather than reverting to the unscaled default (SDD-016's R20d.8.10 amendment, finding 242's climate half) |
...
| R12b-V18 | A build-up test shuts its wells in for real | Ordering a well test closes every well on the target compartment immediately, refuses against a compartment with none or with one already shut in, and the wells stay shut after the test completes — production for that tick is zero, and a belief measured before a year of shut-in does not go stale, which it would if the shut-in were flavour text (SDD-007 §5's R12b.18 amendment, finding 245) |

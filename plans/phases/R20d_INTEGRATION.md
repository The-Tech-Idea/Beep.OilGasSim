...
| 4 | **The chain (R20d.1–5)** — with **R12b.8 install** | 🟨 `IFlowSolver` over the registered network; separation, gas, water, transport; custody becomes a metered point. Closes FV1's 1000-tick half, R11-V13, SC7/SC8. An install template is catalogue work the moment a facility is something the loop routes through rather than around. **Started with the element that blocks it**: a separator had no pressure to impose, so the solver would have handed every wellhead the terminal sink boundary (finding 157) |

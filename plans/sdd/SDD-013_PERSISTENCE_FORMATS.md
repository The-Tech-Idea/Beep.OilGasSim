...
| S013-9 | The read model still parts somewhere outside production, cash and the ledger — `FieldReadModel` compares equal only if every collection does, and the chain rows are the likely half. Not isolated. PV2 is met on the numbers that matter and not yet on the whole projection | R20d.12 |

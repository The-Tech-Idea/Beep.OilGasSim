...
| S013-9 | **Two more cumulative totals were unsaved and are now closed**: `CumulativeFlared` — what a company has flared over its life, which the ESG record is scored on and its debt priced against — and `CumulativeProduced`, which the bank lends against. A reload reset both, so a forty-year field came back with the flaring record of a new one: 200,013 tonnes against 3,287. **What is left is `Chain` and only `Chain`**: every other named field on the read model agrees month after month for two years, and the rows still part somewhere while every barrel and every cent matches. The test asserts the exclusion by name, so closing it is deleting one word | R20d.12 |

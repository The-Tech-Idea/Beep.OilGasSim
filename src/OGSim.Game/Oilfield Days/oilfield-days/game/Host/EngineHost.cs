...
public sealed partial class EngineHost : Node
{
		...
		Era StartEra)
	{
		...
		public static NewGameDraft Default(ulong seed) => new(
			seed, GameStyles.Days.Id.Value, "world-template-basin",
			Cells: 24,
			LandFraction: 0.6,
	...
	/// <summary>
	/// Everything this product hands the engine: the base tree, with the style's
	/// own departures stacked over it (order 1 — an overlay entry replaces the
	/// base entry wholesale, SDD-004 §7).
	/// </summary>
	/// <remarks>
	/// One method for the new-game path and the load path both, because the four
	/// axes were once spelled out separately on those two paths and drifted — and
	/// the content list repeated that defect: <see cref="GodotContentSource.StyleOverrides"/>
	/// was shipped with zero callers, so Oilfield Days played at the base game's
	/// export scale ($45M expansions where its own content says $18M, the base
	/// parcel where it says the 8,000 t coaster) — the Engineer's economy inside
	/// the Settlers-shaped game.
	/// </remarks>
	private static IReadOnlyList<IContentSource> ContentFor(GodotContentSource content, IGameStyle style) =>
		GodotContentSource.StyleOverrides(style.Id) is { } overlay
			? [content, overlay]
			: [content];
	...
	public bool NewGame(NewGameDraft draft)
	{
		...
		IGameStyle style = GameStyles.Named(new ContentId(draft.Style));
		IReadOnlyList<IContentSource> sources = ContentFor(content, style);

		GD.Print($"[content] {content.Count} files read from res://content"
			+ (sources.Count > 1
				? $", {sources[1].Files.Count} of them overridden by styles/{style.Id.Value}"
				: string.Empty));
		...
		var settings = style.Compose(new EngineSettings(
	...
	public bool Load(SaveSlots.Slot slot)
	{
		...
		GodotContentSource content = GodotContentSource.Shipped();
		IGameStyle style = GameStyles.Named(new ContentId(slot.Draft.Style));
		...
		var settings = style.Compose(new EngineSettings(
			...
			// The same list the new-game path composes with: a save opened under
			// different content than the run was played under would price the
			// same order two ways (law L5).
			Content: ContentFor(content, style),
			StartingState: style.StartingState,
			Rules: style.Rules,
			Style: style.Id));

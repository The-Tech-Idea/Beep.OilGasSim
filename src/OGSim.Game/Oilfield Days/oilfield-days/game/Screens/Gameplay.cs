...
public sealed partial class Gameplay : Node2D
{
	...
	public override void _Ready()
	{
		...
		_world = RequireNamed<BasinWorld>(this, "Basin");
		...
		_hud = RequireNamed<GameHud>(this, "Hud");
		_hud.Layer = 10;
		...
		var shell = RequireNamed<CanvasLayer>(this, "Shell");
		shell.Layer = 20;

		_status = RequireNamed<StatusBar>(shell, "StatusBar");
		...
		_rail = RequireNamed<IconRail>(shell, "Rail");
		_rail.SetAnchorsAndOffsetsPreset(Control.LayoutPreset.TopLeft);
		_rail.OffsetLeft = 8;
		_rail.OffsetTop = 76;
		_rail.OffsetRight = 102;
		_rail.OffsetBottom = 704;
		...
		_selection = RequireNamed<SelectionCard>(shell, "Selection");
		...
		_side = RequireNamed<SidePanels>(shell, "Side");
		_side.GoTo += LookAt;
		...
		_bar = RequireNamed<CommandBar>(this, "CommandBar");
		_bar.Layer = 11;
		...
		_truck = RequireNamed<ServiceTruck>(this, "ServiceTruck");
		_truck.Position = _world.PlantSite;
		_truck.Bounds = _world.Extent;
		_truck.ControlsEnabled = false;
		...
		_orders = RequireNamed<StandingOrders>(this, "StandingOrders");
		...
		_camera = RequireNamed<CameraRig>(this, "Camera");
	...
	private static T RequireNamed<T>(Node parent, string name) where T : Node =>
		parent.GetNodeOrNull<T>(name) ?? throw new System.InvalidOperationException(
			$"{parent.GetPath()} requires an authored {typeof(T).Name} named '{name}'.");

...
| # | Finding | Status |
|---|---|---|
| **GC-1 — FIXED** | **`FieldReadModel.Prospects` does not filter what has been drilled.** The field is documented as "every structure the world placed that the company has **not drilled**"; the projection filters on `risks.Knows` and nothing else. Seed 3: prospect #1 drilled three times, POS climbing 0.23 → 0.32 → 0.40, count static at seven. The behaviour is defensible — a second well into a found structure is appraisal — but it and the documentation cannot both be right. *Settled by engine finding 286, and the split is exactly the one this row guessed at: a structure drilled and found EMPTY is condemned — off the board, a second hole refused by name (`reject.prospect-condemned`), the save remembers — while a drilled DISCOVERY stays listed, because a second well into a found structure is appraisal and both shipped clients pick infill wells off the list. The doc now says "not condemned". The three re-drills this row measured were the real defect: each one re-counted the same source evidence against the play, which is what was killing seeds 3 and 4 in the balance sweep.* | closed |
| **GC-7 — FIXED** | **The shipped style overlay never reached the composed engine.** `GodotContentSource.StyleOverrides` was written for the two-products landing — order 1, mirroring `RepositoryContent.StyleOverrides` exactly — and shipped with ZERO callers: both `EngineHost` compose paths passed `Content: [content]`, base tree only. So Oilfield Days played at the base game's export scale — $45M/9-tick expansions where its own `content/styles/days/` says $18M/4-tick, the base parcel where it says the 8,000 t coaster — the Engineer's economy inside the Settlers-shaped game, while the console client (`OGSim.Engineer/Program.cs`) stacked the overlay correctly from day one. *Fixed in `EngineHost`: one `ContentFor(content, style)` method both paths call (the same one-place rule `Style.Write` already enforces for the axes, and for the same reason — the two paths had already drifted once), overlay presence logged at boot. A save reopens under the same list the new-game path composes with, because content is the host's to hand over and a save priced under one economy must not reload under another (law L5).* | closed |
